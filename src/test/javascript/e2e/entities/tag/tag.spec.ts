...
describe('Tag e2e test', () => {
  ...
  it('should load Tags', async () => {
    ...
    expect(await tagComponentsPage.getTitle()).to.eq('riverApp.tag.home.title');
  });

  it('should load create Tag page', async () => {
    await tagComponentsPage.clickOnCreateButton();
    tagUpdatePage = new TagUpdatePage();
    expect(await tagUpdatePage.getPageTitle()).to.eq('riverApp.tag.home.createOrEditLabel');
  ...
  it('should delete last Tag', async () => {
    ...
    expect(await tagDeleteDialog.getDialogTitle()).to.eq('riverApp.tag.delete.question');
